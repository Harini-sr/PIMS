...
export class UserDashboardComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    const userId :any= '68f0c19709ab2ee802317327';  // Replace with dynamic user ID as needed
    //const userId=1;
...
//   if (token) {
//    const decodedToken: any = (jwt_decode as any).default(token);
//     const userId = decodedToken.userId || decodedToken._id; // depends on your token payload
...
// }
  ...
  statusClass(status?: string) {
    ...
    if (s.includes('pending') || s.includes('open')) return 'status-open';
